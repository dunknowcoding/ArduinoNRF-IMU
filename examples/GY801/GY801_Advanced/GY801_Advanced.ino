/*
  GY801_Advanced - Configure/calibrate a GY-801 module and show raw reads.
*/
#include <GY801.h>
...
void setup() {
  ...
  if (!board.begin()) {
    Serial.println("GY-801 core sensors not found.");
    while (true) {
      delay(1000);
    }
  }
...
void loop() {
  ...
  bool compassReady = board.magOk() &&
      (board.compassKind() == GY801::CompassKind::HMC5883L
           ? board.hmc().dataReady()
           : board.qmc().dataReady());

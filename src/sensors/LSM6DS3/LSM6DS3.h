/*
  LSM6DS3.h - ArduinoNRF-IMU driver for ST's LSM6DS3 6-axis IMU.
*/
#ifndef ARDUINONRF_IMU_LSM6DS3_H
#define ARDUINONRF_IMU_LSM6DS3_H

#include "../../imu/IMUSensor.h"
#include "LSM6DS3_Registers.h"

namespace nimu {

class LSM6DS3 : public IMUSensor {
 public:
  LSM6DS3() { name_ = "LSM6DS3"; }

  bool begin() override;
  bool beginI2C(TwoWire& wire, uint8_t address) override;
  bool beginSPI(SPIClass& spi, uint8_t csPin) override;
  uint8_t whoAmI() override;
  bool isConnected() override;
  bool update() override;

  bool setAccelRangeG(uint16_t maxG) override;
  bool setGyroRangeDps(uint16_t maxDps) override;
  bool setLowPassFilterHz(uint16_t hz) override;
  bool setSampleRateHz(uint16_t hz) override;

  bool reset();
  bool dataReady();
  bool configureInterruptPins(bool activeLow, bool openDrain);
  bool routeInterrupt(uint8_t pin, uint8_t sources);
  bool setDataReadyInterrupt(uint8_t pin, bool accel = true,
                             bool gyro = true);
  bool enablePedometer(uint8_t interruptPin = 1);
  bool disablePedometer();
  uint16_t stepCount();
  bool resetStepCount();
  bool stepDetected();
  bool enableTiltDetection(uint8_t interruptPin = 1);
  bool disableTiltDetection();
  bool tiltDetected();
  bool enableSignificantMotion(uint8_t interruptPin = 1);
  bool significantMotionDetected();
  bool enableSensorHubPullups(bool enable = true);
  bool setSensorHubPassThrough(bool enable = true);
  bool sensorHubRead(uint8_t address, uint8_t reg, uint8_t* data,
                     uint8_t length, uint16_t timeoutMs = 30);
  bool sensorHubWrite(uint8_t address, uint8_t reg, uint8_t value,
                      uint16_t timeoutMs = 30);
  void setBusClockHz(uint32_t hz);
  uint32_t busClockHz() const { return clockHz_; }

  struct RawSample {
    int16_t ax, ay, az;
    int16_t gx, gy, gz;
    int16_t temp;
  };
  bool readRaw(RawSample& out);

 protected:
  bool configureDefaults();
  uint8_t odrBitsForHz(uint16_t hz, uint16_t& actualHz) const;
  bool embeddedAccess(bool enable);
  bool waitSensorHub(uint16_t timeoutMs);

  uint32_t clockHz_ = 400000;
  uint8_t odrBits_ = lsm6ds3::ODR_104;
  float accelLsbPerG_ = 8192.0f;   // +/-4 g default
  float gyroLsbPerDps_ = 32.768f;  // +/-1000 dps default
  uint16_t accelRangeG_ = 4;
  uint16_t gyroRangeDps_ = 1000;
  uint16_t sampleRateHz_ = 104;
};

}  // namespace nimu

using nimu::IMUCalibration;
using nimu::IMUData;
using nimu::LSM6DS3;
using nimu::Vec3;

#endif  // ARDUINONRF_IMU_LSM6DS3_H

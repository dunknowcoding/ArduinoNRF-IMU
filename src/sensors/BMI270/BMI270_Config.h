/*
  BMI270_Config.h - BMI270 maximum FIFO configuration blob.

  Source: Bosch Sensortec BMI270 SensorAPI bmi270_maximum_fifo.c v2.86.1.
  Copyright (c) 2023 Bosch Sensortec GmbH. All rights reserved.
  Licensed under BSD-3-Clause; retain this notice when redistributing.
*/
#ifndef ARDUINONRF_IMU_BMI270_CONFIG_H
#define ARDUINONRF_IMU_BMI270_CONFIG_H

#include <Arduino.h>

namespace nimu {
namespace bmi270 {

constexpr uint8_t kConfigFile[] = {
  ,
};

}  // namespace bmi270
}  // namespace nimu

#endif  // ARDUINONRF_IMU_BMI270_CONFIG_H
